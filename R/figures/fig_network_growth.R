...
# ---- fig_cumulative_siteyears_igbp ------------------------------------------

#' Cumulative site-years: IGBP-stacked Shuttle area with historical overlays
#'
#' Variant of \code{\link{fig_siteyears_by_year_igbp}} in which annual
#' site-year counts are replaced by cumulative site-years (running total from
#' the first year of \code{year_range} to each year in the range).  Both the
#' Shuttle IGBP background and the historical dataset lines are cumulative,
#' so all series are monotonically non-decreasing.
#'
#' NOTE: historical site lists are comparison/illustration data only —
#' non-Shuttle primary data (see CLAUDE.md §1).
#'
...
#' )
#' # ... load marconi, la_thuile, fluxnet2015 ...
#' p <- fig_cumulative_siteyears_igbp(presence, shuttle, marconi, la_thuile, f15)
#' print(p)
#' }
#' @export
fig_cumulative_siteyears_igbp <- function(presence_df,
                                           shuttle_meta,
                                           sites_marconi,
                                           sites_la_thuile,
                                           sites_fluxnet2015,
  ...
  # ---- Historical: cumulative site-years per dataset -----------------------
  # Expand each site's record to one row per active year, then cumsum.
  .expand_years_cumul <- function(df) {
    valid <- df |>
      dplyr::distinct(.data$site_id, .keep_all = TRUE) |>
      dplyr::filter(!is.na(.data$first_year), !is.na(.data$last_year)) |>
      dplyr::mutate(first_year = as.integer(.data$first_year),
    ...
    do.call(rbind, Map(
      ...
      dplyr::filter(.data$year >= yr_min, .data$year <= yr_max) |>
      dplyr::count(.data$year, name = "n") |>
      # Fill missing years with 0 so cumsum has no gaps
  ...
  # For cumulative lines the peak is always the last non-zero year — use that
  # for direct label placement.
  peak_labels <- hist_all |>
    dplyr::group_by(.data$dataset) |>
    dplyr::slice_max(.data$n, n = 1L, with_ties = FALSE) |>
    dplyr::ungroup()
