...
# ---- Read ICOS likely sites ----
ICOS_likely_df <- readr::read_csv(ICOS_likely, show_col_types = FALSE) |>
  janitor::clean_names()

# Normalize latitude/longitude column names to latitude/longitude
lat_candidates <- c("latitude", "lat", "y", "decimal_latitude")
lon_candidates <- c("longitude", "lon", "long", "x", "decimal_longitude")

lat_col2 <- intersect(lat_candidates, names(ICOS_likely_df))[1]
lon_col2 <- intersect(lon_candidates, names(ICOS_likely_df))[1]
if (is.na(lat_col2) || is.na(lon_col2)) {
  stop("ICOS_likely_df: Could not find latitude/longitude columns.")
}

ICOS_likely_df <- ICOS_likely_df |>
  dplyr::rename(latitude = !!lat_col2, longitude = !!lon_col2) |>
  dplyr::mutate(
  ...
  dplyr::filter(dplyr::between(latitude, -90, 90),
...
# ---- Export CSV: points used in "Global Potential Network" ----
dir.create("outputs", showWarnings = FALSE, recursive = TRUE)

# Choose an identifier column for the output "site" field
id_candidates <- c("site_code", "site_id", "site", "site_name")
id_col <- id_candidates[id_candidates %in% names(sites)][1]
if (is.na(id_col)) stop("No site identifier column found in `sites` (expected one of site_code/site_id/site/site_name).")

# Build FLUXNET2015 coordinate set (exactly what is plotted as the overlay)
f15_xy <- sites_f15 %>%
  dplyr::transmute(latitude, longitude) %>%
  dplyr::distinct() %>%
  dplyr::mutate(in_fluxnet2015 = TRUE)

# Output table: all plotted points + flag if they are in the FLUXNET2015 overlay
sites_out <- sites %>%
  dplyr::mutate(site = as.character(.data[[id_col]])) %>%
  dplyr::left_join(f15_xy, by = c("latitude", "longitude")) %>%
  dplyr::mutate(in_fluxnet2015 = dplyr::coalesce(in_fluxnet2015, FALSE)) %>%
  dplyr::transmute(site, latitude, longitude, in_fluxnet2015) %>%
  dplyr::distinct() %>%
  dplyr::arrange(site)

readr::write_csv(sites_out, file.path("outputs", "global_potential_network_sites.csv"))
...
readr::write_csv(sites_amf_reg, file.path("outputs", "sites_amf_Fluxnet_reg_sites.csv"))
...
# 1) Record length from annual (after dedup/QC)
record_len <- annual %>%
  distinct(site, year) %>%
  count(site, name = "n_years")

# 2) One row per site with coords + record length
annual_sites_len <- annual %>%
  filter(!is.na(LOCATION_LAT), !is.na(LOCATION_LONG), !is.na(IGBP)) %>%
  distinct(site, latitude = LOCATION_LAT, longitude = LOCATION_LONG) %>%
  mutate(longitude = ifelse(longitude > 180, longitude - 360, longitude)) %>%
  left_join(record_len, by = "site")

# 3) Map: all sites background + analysis sites sized by n_years
p1_all_blue_analysis_len <- outline_base("Global — Sites Used (Sized by Record Length)") +
  # background: all potential sites
  geom_point(
    data  = sites,
    aes(x = longitude, y = latitude),
    shape = 21,
  ...
  # overlay: analysis sites, size = record length
  geom_point(
    data  = annual_sites_len,
  ...
  scale_size_continuous(
    name = "Record length (years)",
    range = c(2.2, 7.5),   # tweak to taste
    breaks = pretty_breaks(5)
  ) +
  theme(legend.position = "bottom")

p1_all_blue_analysis_len

p1_all_blue_analysis_len_col <- outline_base(
  "Global — Sites Used in Current Analysis (Record Length)"
) +
  # background: all potential sites
  geom_point(
    data  = sites,
  ...
  # overlay: analysis sites colored by record length
  geom_point(
    data  = annual_sites_len,
    aes(x = longitude, y = latitude, fill = n_years),
    shape  = 21,
    color  = "black",
    size   = pt_size + 0.4,
    alpha  = 0.95,
...
p1_all_blue_analysis_len_col
p1_all_blue_analysis_len_col_cyan <- p1_all_blue_analysis_len_col +
  scale_fill_gradient(
    name = "Record length (years)",
    low  = "azure2",
    high = cyan_f15
...
p1_all_blue_analysis_len_col_cyan
...
# Optional saves:
 ...
 ggsave("maps/map_4_all_blue_plus_FLUXNET_analysis_length.png", p1_all_blue_analysis_len_col_cyan, width = 10, height = 6, dpi = 300)
 
 # Optional save
 ggsave(
   "maps/map_1_5_analysis_sites_overlay.png",
   p1_all_blue_analysis,
...
# Optional saves:
 ...
 # ---- Export CSV: data used in "Global Potential Network" map ----
 
 # Use site_code as the site identifier if present; otherwise fall back to site_name
 if (!("site_code" %in% names(sites)) && !("site_name" %in% names(sites))) {
   stop("Expected 'site_code' or 'site_name' in sites. Check names(sites).")
 }
 
 sites_out <- sites %>%
   mutate(
     site = dplyr::coalesce(
       dplyr::if_else("site_code" %in% names(sites), as.character(site_code), NA_character_),
       dplyr::if_else("site_name" %in% names(sites), as.character(site_name), NA_character_)
     )
   ) %>%
   transmute(
     site,
