...
#####Discover ICOS files
discover_ICOS_files <- function(data_dir = "data/FLUXNET/ICOS") {
  library(fs)
  library(stringr)
  library(dplyr)
  library(tidyr)
  ...
  # 2a. FLX_* legacy FLUXNET2015 (FULLSET / SUBSET / ERAI, plus AUX)
  flx <- tibble(path = all_csvs) %>%
    mutate(filename = basename(path))
  
  # FULLSET / SUBSET / ERAI with explicit time integral
  std_pat <- 
    "^(FLX)_([^_]+)_(FLUXNET2015)_(FULLSET|SUBSET|ERAI)_(DD|HH|MM|WW|YY_INTERIM|YY)_([0-9]{4})-([0-9]{4})_.*\\.csv$"
  
  flx_std <- flx %>%
    filter(str_detect(filename, std_pat)) %>%
    tidyr::extract(
      col   = filename,
  ...
  # AUXMETEO / AUXNEE (no explicit time integral)
  aux_pat <- 
    "^(FLX)_([^_]+)_(FLUXNET2015)_(AUXMETEO|AUXNEE)_([0-9]{4})-([0-9]{4})_.*\\.csv$"
  
  flx_aux <- flx %>%
    filter(str_detect(filename, aux_pat)) %>%
    tidyr::extract(
      col   = filename,
      into  = c("data_center","site","data_product",
    ...
    mutate(time_integral = NA_character_)
  
  flx_manifest <- bind_rows(flx_std, flx_aux) %>%
    mutate(
      across(c(data_center, site, data_product, dataset, time_integral),
             as.character),
      across(c(start_year, end_year), as.integer)
  ...
  # 2b. Modern ICOSETC_* L2 files
  icos_manifest <- tibble(path = all_csvs) %>%
    filter(str_detect(basename(path), "^ICOSETC_[^_]+_FLUXNET")) %>%
    mutate(filename = basename(path)) %>%
    tidyr::extract(
      col   = "filename",
...
organize_fluxnet_files <- function(manifest, base_out) {
  manifest2 <- manifest %>%
    mutate(
      dest_dir = case_when(
        # AUXMETEO/AUXNEE (no time integral)
        is.na(time_integral) & dataset %in% c("AUXMETEO", "AUXNEE") ~
          file.path(base_out, dataset),
        
        # ERA reanalysis (ERAI etc) → separate ERAI_* folders
        !is.na(time_integral) & dataset %in% c("ERAI", "ERA5") ~
          file.path(base_out, paste0(time_integral, "_", dataset)),
        
        # Everything else: e.g. DD_FULLSET, YY_L2, etc.
  ...
  # move with overwrite-newer logic
  manifest2 %>%
    rowwise() %>%
    do({
      src  <- .$path
