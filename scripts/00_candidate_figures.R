...
## lubridate, scales, grDevices.
## All sections use ggplot2 rendered to static PNG saved in review/figures/.
...
<header>
  ...
  <p>Paper-candidate figures under development &mdash; generated by 00_candidate_figures.R &mdash;
     static charts: ggplot2/PNG &mdash; all figures saved to review/figures/</p>
</header>
<main>
'
...
build_s1 <- function() {
  ...
  if (has_col("NEE_VUT_REF")) {
    ...
    ggplot2::ggsave(nee_path, plot = p, width = 10, height = 6,
                    units = "in", dpi = 150, bg = "white")
    message("Review figure saved: ", nee_path)
    plots[["nee"]] <- paste0('<div class="plot-wrap">', img_tag(nee_path), "</div>")
  }
  ...
  if (has_col("GPP_NT_VUT_REF") || has_col("GPP_DT_VUT_REF")) {
    ...
    p <- ggplot(gpp_long,
      ...
      fluxnet_theme()
    gpp_path <- file.path("review", "figures", "fig_yy_gpp.png")
    ggplot2::ggsave(gpp_path, plot = p, width = 10, height = 6,
                    units = "in", dpi = 150, bg = "white")
    message("Review figure saved: ", gpp_path)
    plots[["gpp"]] <- paste0('<div class="plot-wrap">', img_tag(gpp_path), "</div>")
  }
  ...
  if (has_col("LE_F_MDS")) {
    p <- ggplot(dplyr::filter(df, !is.na(.data$LE_F_MDS)),
      ...
      labs(title = "Annual ET (from LE_F_MDS) \u2014 all sites (coloured by IGBP)",
           x = "Year", y = "ET (mm yr\u207b\u00b9)") +
      fluxnet_theme()
    et_path <- file.path("review", "figures", "fig_yy_et.png")
    ggplot2::ggsave(et_path, plot = p, width = 10, height = 6,
                    units = "in", dpi = 150, bg = "white")
    message("Review figure saved: ", et_path)
    plots[["et"]] <- paste0('<div class="plot-wrap">', img_tag(et_path), "</div>")
  }
...
build_s2 <- function() {
  ...
  make_panel <- function(aes_y, title, ylab, slug) {
    ...
    out_path <- file.path("review", "figures", paste0("fig_mm_", slug, ".png"))
    ggplot2::ggsave(out_path, plot = p, width = 10, height = 6,
                    units = "in", dpi = 150, bg = "white")
    message("Review figure saved: ", out_path)
    paste0('<div class="plot-wrap">', img_tag(out_path), "</div>")
  }
  ...
  if (!all(is.na(clim$GPP_NT)) || !all(is.na(clim$GPP_DT))) {
    ...
    p_gpp <- ggplot(gpp_long,
      ...
      fluxnet_theme() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    gpp_mm_path <- file.path("review", "figures", "fig_mm_gpp.png")
    ggplot2::ggsave(gpp_mm_path, plot = p_gpp, width = 10, height = 6,
                    units = "in", dpi = 150, bg = "white")
    message("Review figure saved: ", gpp_mm_path)
    plots[["gpp"]] <- paste0('<div class="plot-wrap">', img_tag(gpp_mm_path), "</div>")
  }
