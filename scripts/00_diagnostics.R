...
## lubridate, scales, grDevices, jsonlite, plotly, leaflet.
## Time-series and climatology sections use plotly (interactive).
## Site map uses leaflet (OpenStreetMap tiles).
## QC / sensitivity sections use ggplot2 inline SVG (static).
...
# Render a ggplot to an inline SVG string (used for static QC/sensitivity plots).
...
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"
  integrity="sha256-20nQCchB9co0qIjJZRGuk2/Z9VM+kNiyxNV/XN/WvsE="
  crossorigin=""></script>
...
<header>
  ...
  <p>Interactive report generated by 00_diagnostics.R &mdash;
     time-series/climatology charts: plotly &bull; site map: leaflet &bull;
     QC/sensitivity charts: ggplot2/SVG</p>
</header>
<main>
'
...
build_s1 <- function() {
  ...
  # Site map — leaflet with OpenStreetMap tiles
  map_html <- if (!is.null(snapshot_meta) &&
    ...
    if (nrow(map_df) > 0) {
      ...
      markers_json <- jsonlite::toJSON(
        data.frame(lat = map_df$location_lat, lon = map_df$location_long,
                   popup = popup_text, site = map_df$site_id,
                   stringsAsFactors = FALSE),
        dataframe = "rows", auto_unbox = TRUE
      )
      paste0(
        '<div id="site-map" style="height:450px;border:1px solid #e0e0e0;',
        'border-radius:4px;margin-bottom:1rem"></div>\n',
        '<script>(function(){\n',
        '  var map=L.map("site-map").setView([20,0],2);\n',
        '  L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",',
        '{attribution:"&copy; <a href=\\"https://www.openstreetmap.org/copyright\\">',
        'OpenStreetMap</a> contributors",maxZoom:18}).addTo(map);\n',
        '  var markers=', markers_json, ';\n',
        '  markers.forEach(function(m){\n',
        '    L.circleMarker([m.lat,m.lon],{radius:8,color:"#1a1a2e",',
        'fillColor:"#4fc3f7",fillOpacity:0.8,weight:1.5})\n',
